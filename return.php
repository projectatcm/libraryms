	...
	function getBookInfo(book_id){
		...
		$.get('get_book.php?book_id='+book_id,function(data){
			...
			if(book.length == 0){
				...
				if(status == 0){
					var taken_by = book.taken_by;
					getStudentInfo(taken_by);
				}
				...
				// fetching fine data 
				$.get('get_fine.php?id='+book_id,function(data){
					data = data.trim();
					if(data != 0){
						$('#fine').text("Fine : ₹ "+data);
					}
...
<div class="container-fluid">
	<div class="card">
		<div class="header">
			<h4 class="title text-center">Return Book</h4>
		</div>
		<div class="content">
		</div>
	</div>
				...
				<div class="content">
					...
					<div class="row">
						<div class="col-sm-12">
							...
							<label id="fine" style="font-size: 45px; margin:0px 20px;"></label>
						</div>
					</div>
				</div>
	...
	<div class="row text-center" style="padding-bottom:15px;">
		<div class="col-xs-12">
			...
			<form class="" action="actions.php?action=retrunBook" method="post">
				<div class="form-group hidden">
					<label>Book Id</label>
					<input type="text" name="lend_bookid" id="lendbookidField" class="form-control" value="">
				</div>

				<button type="submit" class='btn btn-primary' id="lendbookbtn" name="button">Return</button>
			</form>
